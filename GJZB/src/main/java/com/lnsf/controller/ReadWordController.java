...
* @introduction    
* （1）下载文件接口：/Resourcesdownload.do?path=文件名称（自动去D盘的homework下边）
* （2）读取文件接口：/ReadWord/readWord   接受post参数：filePath
* （3）文件上传   
*/
@Controller
...
public class ReadWordController {
	...
	* @author 黄卉 
	* @version 创建时间：2017年7月31日 下午8:26:27
	* @introduction      读取文档到jsp中
	*f://1.2 论文.doc
	*/
	@ResponseBody
	//@RequestMapping(value="/readWord",produces="text/html; charset=UTF-8")//f://1.2 论文.doc
	@RequestMapping(value="/readWord", method = RequestMethod.POST)//f://1.2 论文.doc
	...
	*/
	 //-------------------------------------------------------
		...
		*/
		...
		public String addProjectResource(@ModelAttribute Project project,
			...
			try {
				...
				if (!("").equals(fileName)) {
					...
					newFileName = project.getProjectId()+ date + ".doc";
