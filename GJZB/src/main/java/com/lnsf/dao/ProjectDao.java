...
public interface ProjectDao {
	...
	*/
	@Insert("insert into gjzb_project values(projectseq.nextval,"
	...
	*/
	...
	//根据Id修改所有项目信息
	@Update("update gjzb_project "
			+ "set fkuserId  =#{user.userId}"
			+ ", fktypeId =#{type.typeId}"
	...
	@Select("select p.* ,rownum from gjzb_project p where projectId in("
			+"select projectId from gjzb_project minus select fkprojectId from gjzb_win) and rownum<=5 order by ReleaseTime desc")
