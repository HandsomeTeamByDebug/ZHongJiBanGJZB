...
<script type="text/javascript">
	...
	<div id="container"
		...
		<div id="c_left" style="width: 200px; height: 600px; float: left;">
			<form action="${pageContext.request.contextPath }/users/addOneUserPicture" method="post" enctype="multipart/form-data" id="picForm">
					<input type="hidden" name="userId" id="userId" value="${user.userId }"/>
				<img src="<%=htph %>/image/image/${user.profilePic}" width="190px" id="img"/><br>
				<div style="margin-top: 10px;">
					<a href="javascript:void(0);" style="padding: 4px 10px;height: 50px;line-height: 50px;position: relative;border: 1px solid #999;text-decoration: none;color: while;">修改头像 
					<input
						style="position: absolute;overflow: hidden;right: 0;top: 0;opacity: 0;" type="file" name="file" accept="image/*" onchange="openFile(this)"/>
					</a>
				</div>
			</form>
		</div>
		<div id="c_right" style="width: 600px; height: 600px; float: left;">
			...
			<form action="${pageContext.request.contextPath }/users/updateUser" method="post" id="updateForm">
				<input type="hidden" name="userId" id="userId" value="${user.userId }"/>
				<TABLE  border=1 cellSpacing=1 cellPadding=4 align=center>
					<TBODY>
						<TR>
							<TD height=30 width="15%" align=right>用户名：</TD>
							<TD width="35%"><input type="text" name="userName" value="${user.userName }"/></TD>
						</TR>
						<TR>
							<TD height=30 width="15%" align=right>自我介绍：</TD>
							<TD class=category width="35%"><textarea id="introduction" name="introduction" value="${user.introduction }"
									cols="50" rows="10" maxlength="250" placeholder="长度不能超过250字"></textarea>
							</TD>
						</TR>
						<tr>
